...
// DO-178C Level A | True Press v3.8.2

// Fix: encoding UTF-8 forçado em todos os proxies (♦♦ → ç, ã, é)

import { getFeedsByPriority, MAX_ARTICLES_PER_CYCLE, GEMINI_DELAY_MS, FeedSource } from './rssFeeds';
...
const PROXIES = [
...

  {
    ...
    fetch: async (rssUrl: string): Promise<string> => {
      ...
      // &charset=utf-8 força decodificação correta no allorigins

      const res = await fetch(

        `https://api.allorigins.win/get?charset=utf-8&url=${encodeURIComponent(rssUrl)}`,
...

  {
    ...
    fetch: async (rssUrl: string): Promise<string> => {
      ...
      // FIX: arrayBuffer + TextDecoder força UTF-8
...
const parseXML = (xmlContent: string, source: FeedSource): RawNewsItem[] => {
  ...
  // FIX: text/html respeita charset declarado no XML
...
const fetchOneFeed = async (source: FeedSource): Promise<RawNewsItem[]> => {
  ...
  for (const proxy of shuffled) {
    ...
    try {
      ...
      const items = proxy.isJson

        ? parseRss2JsonItems(raw, source)
...
export const harvestGoogleNews = async (query: string): Promise<RawNewsItem[]> => {
  ...
  const fakeSource: FeedSource = {

    name: 'Google News',

    url: googleRssUrl,

    category: 'Manchetes & Alertas',
